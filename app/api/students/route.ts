...
import { listStudents, createStudent } from '@/lib/services/students.service';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const student = await createStudent(session, schoolId, parsed.data);
    return apiOk(student, 201);
  } catch (err) {
    const status = (err as Error & { status?: number }).status;
    if (status === 401) return apiError('Unauthorized', 401);
    if (status === 403) return forbidden();
    return apiError(prismaError(err, 'Failed to create student'), 400);
  }
}
